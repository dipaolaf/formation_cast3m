...
\begin{frame}{\fe{Thermique}{Thermal analysis}}{\fe{Rappels}{Reminders}}
  \begin{itemize}
    ...
    \begin{block}{\fe{Forme locale}{Local form}}
      \begin{equation*}
        \rho c_p \frac{\partial T}{\partial t}+\tx{div}\underbrace{\left(-\lambda\tou{\tx{grad}}(T)\right)}_{\tou{\phi}}=q\quad\fe{\tx{sur }V}{\tx{on }V}
      \end{equation*}
    \end{block}~
    ...
    \fe{\begin{tabular}{lrll}
          ...
          Flux imposés          & $\tou{\phi}.\tou{n}$ & $=\phi_{\tx{imp}}+\underbrace{h(T_f-T)}_{\tx{convection}}+\underbrace{\varepsilon \sigma(T_\infty^4-T^4)}_{\tx{\fe{rayonnement}{radiation}}}$ & sur $\partial V_{\phi}$\\
        \end{tabular}}
       {\begin{tabular}{lrll}
          Imposed temperatures  & $T$                  & $=T_{\tx{imp}}$ & on $\partial V_T$\\
          Imposed heat flow     & $\tou{\phi}.\tou{n}$ & $=\phi_{\tx{imp}}+\underbrace{h(T_f-T)}_{\tx{convection}}+\underbrace{\varepsilon \sigma(T_\infty^4-T^4)}_{\tx{\fe{rayonnement}{radiation}}}$ & on $\partial V_{\phi}$\\
        \end{tabular}}
    \normalsize
    \item \fe{Avec}{With}
    \footnotesize
