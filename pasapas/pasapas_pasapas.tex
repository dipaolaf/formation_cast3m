...
\begin{frame}{\fe{Algorithme de PASAPAS}{PASAPAS algorithm}}
  ...
  \fe{\node (start0) [note, right of=start, xshift=5cm] {\kwo{PAS\_DEFA} (lecture des options),\\
     ...
     {\node (start0) [note, right of=start, xshift=5cm] {\kwo{PAS\_DEFA} (reading options),\\
  ...
  \fe{\node (init0) [note, right of=init, xshift=5.28cm] {\kwo{PAS\_MODL PAS\_ETAT PAS\_MATE} Mise à jour modèle, matériau, chargement};}
     {\node (init0) [note, right of=init, xshift=4.9cm] {\kwo{PAS\_MODL PAS\_ETAT PAS\_MATE} Updating model, properties, load};}

  \fe{\node (resot) [rstep, below of=init] {\g{Résolution Thermique}};}
     {\node (resot) [rstep, below of=init] {\g{Thermal solving}};}
  \fe{\node (resot0) [note, right of=resot, xshift=5.1cm] {\kwo{TRANSNON} Solveur thermique sur le pas de temps\\
                                                           (aussi dispo : \kwo{TRANSLIN}, \kwo{DUPONT2})\\
                                                           Écriture des résultats dans \kw{tab1.}\kwg{'ESTIMATION'}};}
     {\node (resot0) [note, right of=resot, xshift=4.9cm] {\kwo{TRANSNON} Thermal solver on the time step\\
  ...
  \fe{\node (resom0) [note, right of=resom, xshift=5.08cm] {\kwo{UNPAS} Solveur mécanique sur le pas de temps\\
                                                           Écriture des résultats dans \kw{tab1.}\kwg{'ESTIMATION'}};}
     {\node (resom0) [note, right of=resom, xshift=4.87cm] {\kwo{UNPAS} Mechanical solver on the time step\\
                                                           Saving results in \kw{tab1.}\kwg{'ESTIMATION'}};}
  ...
  \fe{\node (sauv0) [note, right of=sauv, xshift=4.93cm] {\kwo{PAS\_RESU} : remplissage de \kwg{'CONTINUATION'} d'après \kwg{'ESTIMATION'}\\
                                                         écriture dans \kw{tab1} s'il s'agit d'un \kwg{'TEMPS\_SAUVE'}};}
     {\node (sauv0) [note, right of=sauv, xshift=4.8cm] {\kwo{PAS\_RESU}: filling \kwg{'CONTINUATION'} according to \kwg{'ESTIMATION'}\\
...
\begin{frame}{\fe{Accès aux données de PASAPAS}{Acces to PASAPAS data}}
  \begin{itemize}
    ...
    \begin{tabular}{ll}
      ...
      etc\dots&\\
    \end{tabular}
    \\~\\ \fe{Plus d'infos, voir les commentaires procédure \kwo{PAS\_DEFA}}
